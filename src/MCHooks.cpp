...
DeclareHook(readFile, std::string*, void* This, std::string* retstr, Core::Path& path) {
    ...
    if (brd::Options::materialBinLoaderEnabled && brd::Options::redirectShaders && resourcePackManager) {
...
DeclareHook(clientInstance_Update, __int64, void* This, unsigned __int8 isInitFinished) {
    using func_t = void*(*)(void*);
    static uintptr_t adrr = FindSignatures(
        // ClientInstance::getResourcePackManager
        // 1.21.120
        "48 8B ? ? ? ? ? 48 8B ? 48 8B ? ? ? ? ? 48 FF ? ? ? ? ? CC CC CC CC CC CC CC CC"
        " 48 8B ? ? ? ? ? 48 8B ? 48 8B ? ? ? ? ? 48 FF ? ? ? ? ? CC CC CC CC CC CC CC CC"
...
DeclareHook(RayTracingResourcesConstructor,
...
DeclareHook(RayTracingResourcesConstrucstor, void, void *_this) {
    original(_this);
    if (!gDeferredParams) {
        gDeferredParams = (dragon::framerenderer::DeferredShadingParameters *)((int64_t)_this + 176);
    }
}
...
void initMCHooks() {
    ...
    discardFrame = (PFN_mce_framebuilder_BgfxFrameBuilder_discardFrame)
        FindSignatures(
            ...
            "57 48 81 EC 90 00 00 00 48 8B 05 ? ? ? ? 48 33 C4 48 89 84 24 ? ? ? ? 44 0F B6 EA",
            // 1.21.111
    ...
    freeShaderBlobs = (PFN_dragon_materials_CompiledMaterialManager_freeShaderBlobs)
            FindSignatures(
                // 1.21.130
                "48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 54 41 55 41 56 41 57 48 83 EC 30 4C 8B E9 48 83 C1 40",

                "48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 54 41 55 41 56 41 57 48 83 EC 20 4C 8B E9 48 83 C1 40"
            );
    if (!freeShaderBlobs) {
        Logger::log("dragon::materials::CompiledMaterialManager::freeShaderBlobs not found");
    }
    ...
    TrySigHook(mce_framebuilder_BgfxFrameBuilder_endFrame,
        ...
        // 1.21.130
        "48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? B8 20 1D 00 00",
        // 1.21.120
        "48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? B8 10 1D 00 00"
    );
    TrySigHook(HOOK1,
        // 1.21.100
    ...
    TrySigHook(RayTracingResourcesConstructor,
        // 1.21.100
        "48 89 5C 24 ? 48 89 6C 24 ? 56 57 41 56 48 83 EC 50 0F 29 74 24 ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 44 24 ? 4D 8B F1"
    );
    TrySigHook(RayTracingResourcesConstrucstor,
        // 1.21.100
        "48 89 5C 24 ? 48 89 74 24 ? 48 89 4C 24 ? 57 48 83 EC 20 48 8B F9 33 F6 48 89 31 48 89 71 ? C7 41"
    );
}